...
double calculateYearlyCompoundInterestSP500(double principal) {
  const avgTotalYearlyReturn = 0.08;
  return toPrecision(calculateCompoundInterest(principal, avgTotalYearlyReturn, 12, 1));
}
...
List<FlSpot> generateCompoundInterestData(double valuePerInterval,
  ...
  return xData.fold<List<FlSpot>>(
      initialData,
          (value, element) {
        var lastOrNull = value.lastOrNull;
        if (lastOrNull == null) {
          value.add(FlSpot(element,
              calculateYearlyCompoundInterestSP500(valuePerInterval)
          ));
        } else {
          value.add(FlSpot(
              element,
